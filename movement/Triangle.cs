...
namespace shapes
{
    internal class Triangle : DisplayObject
    {
        ...
        protected int[] coordsX_s, coordsY_s;
            ...
            this(SetBaseProperties(point1X, point1Y, point2X, point2Y, point3X, point3Y), R, G, B)
        {
            ...
            coordsX_s = [point1X, point2X, point3X];
            coordsY_s = [point1Y, point2Y, point3Y];
            tag = "tri";
        }
        ...
        protected override void RenewPoints(int dx, int dy)
        {
            for (int i = 0; i < coordsX.Length; i++)
            {
                ...
                coordsX_s[i] = coordsX[i];
                coordsY_s[i] = coordsY[i];
            }
        }
        ...
        protected override void ShiftPoints(int dx, int dy)
        {
            ...
            for (int i = 0; i < coordsX.Length; i++)
            {
                coordsX[i] = coordsX_s[i] + dx;
                coordsY[i] = coordsY_s[i] + dy;
                Console.WriteLine("started: {0}:{1}", coordsX_s[i], coordsY_s[i]);
            }
        }
    }
}
